...
class Graficar_tikz():
                        ...
                        if(figura=="rectangle"):
                            ...
                            #Si no hay...
                                ...
                                with area_de_dibujar.canvas:
                                    #BORDE RECTANGULO
                                    area_de_dibujar.color = Color(self.draw[0], self.draw[1], self.draw[2])
                                    Line(points=[punto_inicial_x_y, punto_final_x_y, punto_inicial_x_y, punto_final_x_y+alto, punto_inicial_x_y, punto_final_x_y+alto, punto_inicial_x_y+ancho, punto_final_x_y+alto, punto_inicial_x_y+ancho, punto_final_x_y+alto, punto_inicial_x_y+ancho, punto_final_x_y,punto_inicial_x_y+ancho, punto_final_x_y, punto_inicial_x_y, punto_final_x_y],width=self.line_width)
                                    #RELLENO RECTANGULO
                                    area_de_dibujar.color = Color(self.fill[0], self.fill[1], self.fill[2])
    ...
    def Validar_estilos(self,tipo_dato,valor,valor_diccionario=None):
        if tipo_dato == "tupla":
            ...
            if isinstance(valor,list):
                for estilo in valor:
                    estilo = estilo.strip()
                    color_porcentaje = estilo.split("!")
                    if estilo in self.colores_validos:
                        self.draw = self.Color(estilo)
                    elif estilo in self.tipo_de_linea_validos:
                        self.tipo_de_linea = estilo
            ...
            if isinstance(valor,list):
                ...
                for key in keys_diccionario:
                    ...
                    if key.find("line width")!=-1:
                        ...
                        self.line_width = self.Validar_metrica(valor_diccionario[key_original])
                    elif key.find("fill")!=-1:
                        #Validar si es correcto el color utilizado...
                        relleno = valor_diccionario[key_original].strip()
                        color_porcentaje = relleno.split("!")
                        if relleno in self.colores_validos:
                            self.fill = self.Color(relleno)
                        elif len(color_porcentaje)==3:#green!70!blue
                            self.Color_porcentaje(color_porcentaje,"fill")
                    elif key.find("draw")!=-1:
                        #Validar si es correcto el color utilizado...
                        borde_color = valor_diccionario[key_original].strip()
            ...
            #VARIABLE CON PARAMETROS
                ...
                if key.find("line width")!=-1:
                    ...
                    color_porcentaje = relleno.split("!")
                    if relleno in self.colores_validos:
                        self.fill = self.Color(valor_diccionario)
                    elif len(color_porcentaje)==3:#green!70!blue
                        self.Color_porcentaje(color_porcentaje,"fill")
                elif key.find("draw")!=-1:
                    #Validar si es correcto el color utilizado...
    ...
    def Color_porcentaje(self,color_porcentaje,tipo_estilo="draw"):
        ...
        if porcentaje > 100 or porcentaje < 0:
            ...
            if len(color_1) == 3 and len(color_2) == 3:
                ...
                for color in color_2:
                    ...
                    indice+=1
                if tipo_estilo == "draw":
                    self.draw = color_elegido
                elif tipo_estilo == "fill":
                    self.fill = color_elegido