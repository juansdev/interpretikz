...
class GuardarDibujoEnImagen():
    ...
    def figura_a_png(self,generar_dibujo_en_formato,name_figure,size,pos,color_relleno,tipo_de_linea,coords_borde,color_borde,line_width,angle_start=0,angle_end=0): 
        ...
        if generar_dibujo_en_formato:
            ...
            ruta = './Pytikz/source/crear_imagen/grafica_original/'+nombre_img
            ...
            ruta = './Pytikz/source/crear_imagen/grafica_recortada/'+nombre_img
    ...
    def crear_imagen(self):
        ...
        btn_salir = MDRaisedButton(text="Vale",font_name="media/fonts/OpenSans-SemiBold")
        ...
        def cerrar_md_dialog(md_dialog,*args):
            md_dialog.dismiss()
        btn_salir.bind(on_press=partial(cerrar_md_dialog,self.md_dialog))
        self.md_dialog.open()
    ...
    def __crear_imagen(self):
        ...
        input_png_list = glob.glob("./Pytikz/source/crear_imagen/grafica_recortada/*.jpg")
        input_png_list.sort()
        ...
        if (len(clips) > 0):
            ...
            if len(clips) == 1:
                concat_clip.write_gif('./imagen_generado_id-'+id+'.jpg', fps=2, logger=my_bar_logger)
            #Es una secuencia de imagenes - GENERAR GIF
            else:
                concat_clip.write_gif('./imagen_generado_id-'+id+'.gif', fps=2, logger=my_bar_logger)
        
        #Si no lo hay es un error...
        else:
            self.md_dialog.ids.container.title = "¡ERROR!"
            self.contenido_progreso_wid.ids.info.text = f"Ocurrio un error al momento de crear la imagen"
        
        #Limpiar recursos
        self.__limpiar_recursos()
    ...
    def actualizar_wid(self, porcentaje_de_realizacion, info="",index=0, total=0, generar_dibujo_en_formato=False,*args):
        ...
        if porcentaje_actual != self.old_value and porcentaje_actual % 5 == 0:
            ...
            if(porcentaje_actual == 100):
                self.md_dialog.ids.container.title = "¡EXITO!"
                self.contenido_progreso_wid.ids.info.text = f"La imagen se creo satisfactoriamente"
            
    def __limpiar_recursos(self):
