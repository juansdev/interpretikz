...
from kivy.properties import OptionProperty, BooleanProperty, NumericProperty, ListProperty, StringProperty
    ...
    *Internal Logic*
    ...
    def select_shape(self, shape):
        ...
        if shape.select():
            ...
            #Figuras seleccionadas - Cambiar borde y el orden de capas de una figura, ademas de borrar, etc. 
            shape.paint_widget.selected_shapes = self.selected_shapes
            #Figuras seleccionadas - Con el proposito de pintarlas
            if len(self.selected_shapes):
                shape.paint_widget.selected_shapes_for_fill = self.selected_shapes
            return True
        return False
    ...
    def on_touch_up(self, touch):
        ...
        if ud['paint_touch_moved']:
            ...
            if ud['paint_was_selected'] and \
                ...
                self.clear_selected_shapes()
            # Actualizar relleno tras soltado
            return {"end":True}
...
class PaintCircle(PaintShape):
    ...
    paint_widget = None
    #Se almacenara la clase de dibujo de la figura en cuestion.

    ready_to_finish = True

    is_valid = True

    def __init__(self, line_width, pointsize , line_color, **kwargs):
        kwargs["line_width"] = line_width if line_width else dp(1)
        kwargs["pointsize"] = pointsize if pointsize else dp(3)
        kwargs["line_color"] = line_color if line_color else (0,0,0,1)
    ...
    def add_area_graphics_to_canvas(self, name, canvas):
        with canvas:
            x, y = self.center
            r = self.radius
            ellipse = Ellipse(size=(r * 2., r * 2.), pos=(x - r, y - r), group=name)

    def add_shape_to_canvas(self, paint_widget):
        if not super(PaintCircle, self).add_shape_to_canvas(paint_widget):
            print("DIBUJAR CIRCULO")
            self.paint_widget = paint_widget
            self.paint_widget.add_colored_shapes_area(self)
            return False
...
class PaintEllipse(PaintShape):
    ...
    def add_shape_to_canvas(self, paint_widget):
        if not super(PaintEllipse, self).add_shape_to_canvas(paint_widget):
            print("DIBUJAR ELIPSE")
            self.paint_widget = paint_widget
            paint_widget.add_colored_shapes_area(self)
            return False
...
class PaintFreeformPolygon(PaintPolygon):
    ...
    def __init__(self, line_width, pointsize , line_color,**kwargs):
        ...
        kwargs["pointsize"] = pointsize if pointsize else dp(3)
        kwargs["line_color"] = line_color if line_color else (0,0,0,1)
        super(PaintFreeformPolygon, self).__init__(**kwargs)
    ...
    def change_border(self,change_border=[],change_color=[]):
        if len(change_color):
            super(PaintFreeformPolygon,self).perim_color_inst = change_color
        else:
            line_width = change_border[0]
            pointsize = change_border[1]
            super(PaintFreeformPolygon,self).perim_color_inst = change_color
            super(PaintFreeformPolygon,self).line_width = line_width
            super(PaintFreeformPolygon,self).pointsize = pointsize
...
class PaintPoint(PaintShape):
    ...
    def __init__(self,  line_width, pointsize , line_color, **kwargs):
        ...
        kwargs["pointsize"] = pointsize if pointsize else dp(3)
        kwargs["line_color"] = line_color if line_color else (0,0,0,1)
        super(PaintPoint, self).__init__(**kwargs)
...
class PaintCanvasBehavior(PaintCanvasBehaviorBase):
    ...
    def create_shape_with_touch(self,line_width,pointsize,line_color, touch):
        ...
        if shape_cls is None:
            return None
        return shape_cls(line_width,pointsize,line_color)
    ...
    def get_shapes(self):
        return super(PaintCanvasBehavior, self).shapes

    def get_before_shape(self,shape,up=False):
        shapes = super(PaintCanvasBehavior, self).shapes
        if up:
            i = shapes.index(shape)+2
        else:
            i = shapes.index(shape)-1
        if i < len(shapes) and i >= 0: 
            return shapes[i]
        elif i == len(shapes):
            return False

    def change_layout(self,shape,before_shape=None):
        if before_shape:
            super(PaintCanvasBehavior, self).reorder_shape(shape,before_shape)
        else:
...
class PainterWidget(PaintCanvasBehavior, FocusBehavior, MDBoxLayout):
    ...
    #Variable guarda los shapes seleccinados
    selected_shapes = []
    selected_shapes_for_fill = ListProperty()
    ...
    pointsize = NumericProperty()

    #Variables de guardar estilos de figuras
    pointsizes = []
    filled_figures = []

    def create_shape_with_touch(self, touch):
        #Reiniciar estilos
        if not self.border_defined:
            self.line_width = dp(1)
            self.pointsize = dp(3)
        else:
            self.border_defined = False
        # self.line_color = [0,0,0,1]
        # self.fill_color = []
        shape = super(PainterWidget, self).create_shape_with_touch(self.line_width,self.pointsize,self.line_color,touch)
        if shape is not None:
            shape.add_shape_to_canvas(self)
        return shape
    ...
    def on_touch_up(self, touch):
        self.mouse = False
        touch_up = super(PainterWidget, self).on_touch_up(touch)
        if isinstance(touch_up,dict):
            touch_up = touch_up["end"]
            self.add_colored_shapes_area(update=True)
        return touch_up
    ...
    def add_colored_shapes_area(self,shape_created=None,update=False):
        #Si no es para actualizar y hay relleno seleccionado...
        if not update and len(self.fill_color):
            #A LA FIGURA RECIEN CREADO APLICAR RELLENO - Se ejecuta al crear una figura
            if shape_created:
                self.filled_figures.append({"shape":shape_created,"fill_color":self.fill_color})
                with self.canvas:
                    color = self.fill_color
                    Color(*color, group='colorful')
                    shape_created.add_area_graphics_to_canvas('colorful', self.canvas)
            #A LA FIGURA SELECCIONADA APLICAR RELLENO - Se ejecuta al tener una figura seleccionada y presionar el boton en cuestion
            else:
                if len(self.selected_shapes_for_fill):
                    color = self.fill_color
                    #Recorrer todos los shape seleccionados
                    for selected_shape in self.selected_shapes_for_fill:
                        index_filled_figures = 0
                        #Recorrer el array self.filled_figures con el proposito de actualizar los valores de relleno de los shape guardados, con el color del relleno establecido (Siempre y cuando el shape seleccionado exista en el array).
                        shape_updated = False
                        for filled_figure in self.filled_figures:
                            shape_old = filled_figure["shape"]
                            if selected_shape == shape_old:
                                shape_updated = True
                                self.filled_figures.remove(filled_figure)
                                self.filled_figures.insert(index_filled_figures,{"shape":selected_shape,"fill_color":color})
                            index_filled_figures+=1
                        #Si el shape seleccionado no existe en el array, agregarlo...
                        if not shape_updated:
                            self.filled_figures.append({"shape":selected_shape,"fill_color":color})
                        #Agregar relleno
                        with self.canvas:
                            Color(*color, group='colorful')
                            selected_shape.add_area_graphics_to_canvas('colorful', self.canvas)
        elif update:
            #Limpiar pintura anterior
            self.canvas.remove_group('colorful')
            #Actualizar los valores del "Shape" en el array de "filled_figuras", eliminando los que ya no existen
            shapes_updated = super(PainterWidget,self).get_shapes()
            index_filled_figures = 0
            for shape_fill in self.filled_figures:
                shape = shape_fill["shape"]
                color = shape_fill["fill_color"]
                try:
                    shapes_updated.index(shape)
                except:
                    filled_figure = {"shape":shape,"fill_color":color}
                    self.filled_figures.remove(filled_figure)

            #Actualizar los valores del "Shape" en el array de "filled_figuras", reordenandolos
            index_filled_figures = 0
            for shape_fill in self.filled_figures:
                #Extraer index del shape organizado
                shape = shape_fill["shape"]
                color = shape_fill["fill_color"]
                index_shape_updated = shapes_updated.index(shape)
                #Ubicar correctamente el Shape en el array, borrando el shape anterior mal ubicado
                filled_figure = {"shape":shape,"fill_color":color}
                self.filled_figures.remove(filled_figure)
                self.filled_figures.insert(index_shape_updated,filled_figure)
                index_filled_figures += 1
    ...
    def change_border(self,number_size=0,change_color=False):
        if len(self.selected_shapes_for_fill):
            if change_color:
                color = self.line_color
            else:
                self.line_width = dp(1) * number_size
                self.pointsize = dp(3) * number_size
                self.border_defined = True
            for selected_shape in self.selected_shapes_for_fill:
                if isinstance(selected_shape,PaintEllipse) or isinstance(selected_shape,PaintCircle):
                    if change_color:
                        selected_shape.ellipse_color_inst.rgb = color[:3]
                    else:
                        selected_shape.line_width = self.line_width
                        selected_shape.pointsize = self.pointsize
                elif isinstance(selected_shape,PaintPolygon):
                    if change_color:
                        selected_shape.perim_color_inst.rgb = color[:3]
                    else:
                        selected_shape.line_width = self.line_width
                        selected_shape.pointsize = self.pointsize
                elif isinstance(selected_shape,PaintPoint):
                    if change_color:
                        selected_shape.color_inst.rgb = color[:3]
                    else:
                        selected_shape.line_width = self.line_width
                        selected_shape.pointsize = self.pointsize
                elif isinstance(selected_shape,PaintFreeformPolygon):
                    if change_color:
                        selected_shape.change_border(change_color=color[:3])
                    else:
                        selected_shape.change_border(change_border=[self.line_width,self.pointsize])

    def set_shape_size(self,text):
        #Obtener valor 
        validate_number = False
        try:
            number_size = int(text)
            validate_number = True
        except:
            print("Painter error: No es un numero valido")
        if validate_number:
            #Añadir estilos a figura a crear
            if not len(self.selected_shapes):
                self.line_width = dp(1) * number_size
                self.pointsize = dp(3) * number_size
                self.border_defined = True
            else:
                self.change_border(number_size)

    def change_edit_mode(self,instance):
        if instance.icon == 'eye':
            self.pointsizes = []
            for shape in self.shapes:
                self.pointsizes.append(shape.pointsize)
                shape.pointsize = dp(0)
        else:
            index_pointsize = 0
            for shape in self.shapes:
                if not len(self.pointsizes) == index_pointsize:
                    shape.pointsize = self.pointsizes[index_pointsize]
                else:
                    break
                index_pointsize+=1

    def change_layer(self,up=False):
        #Debe de haber almenos una figura seleccionada para subir o bajarle de capa
        if len(self.selected_shapes):
            for selected_shape in self.selected_shapes:
                if up:
                    before_shape = super(PainterWidget,self).get_before_shape(selected_shape,up=True)
                else:
                    before_shape = super(PainterWidget,self).get_before_shape(selected_shape)
                if before_shape:
                    super(PainterWidget,self).change_layout(selected_shape,before_shape)
                elif before_shape == False:
                    super(PainterWidget,self).change_layout(selected_shape)